  ...
  <div class="wary_list" @click="colseLayer()" id="clip_num">
    ...
    <van-sticky>
      <van-dropdown-menu class="list_menu">
        ...
        <van-dropdown-item title="筛选" ref="item_screen" title-class="list_item" @close="closeItem({type:'screen'})">
          <p class="pub_tit">推送预警（多选）</p>
          <!--没有开通舆情研判-->
          <ul class="pub_Ul">
            <li :class="parameterType.warningType.val == '' ? 'hover':''" @click="multipleChoice({type:'warningType',val:'',info:'全部'})">全部</li>
            <li :class="(parameterType.warningType.val.indexOf('4') > -1) ? 'hover' : ''" @click="multipleChoice({type:'warningType',val:'4',info:'推送'})" >推送</li>
            <li :class="(parameterType.warningType.val.indexOf('3') > -1) ? 'hover' : ''" @click="multipleChoice({type:'warningType',val:'3',info:'手动预警'})">手动预警</li>
            <li :class="(parameterType.warningType.val.indexOf('2') > -1) ? 'hover' : ''" @click="multipleChoice({type:'warningType',val:'2',info:'专题预警 '})">专题预警 </li>
            <li :class="(parameterType.warningType.val.indexOf('1') > -1) ? 'hover' : ''" @click="multipleChoice({type:'warningType',val:'1',info:'公共预警'})">公共预警</li>
            <li v-show="$store.state.userInfo.judgedState !='1' " :class="(parameterType.warningType.val.indexOf('5') > -1) ? 'hover' : ''" @click="multipleChoice({type:'warningType',val:'5',info:'人工预警'})">人工预警</li>
          </ul>
          <div class="clearfix"></div>
    ...
    <van-list
      ...
      @load="getList">
      <!--研判列表-->
      <div class="pubWary clearfix" v-for="(item,i) of listData" :key="i" @click="judgmentDetail(item)">
        <!--新闻标题-->
        <div class="newTitle flex">
          <!--<router-link tag="a" :to="{path: 'pushwarningdetail', query: { uuid : item.uuid,simhash:item.simhash,infoid:item.infoId}}">{{item.title}}</router-link>-->
    ...
    >
      ...
      <van-datetime-picker
        ...
        :max-date="maxDate"
      />
      <div class="timeButton" @click="screenList({type:'zdytime',val:'',info:''})">完成</div>
    </van-popup>
  </div>
</template>
<script>
  ...
  import {deleteBatchWarning} from '@/api/judgment'
  import {getPushwarningList} from '@/api/pushwarning'
  ...
  export default {
    ...
    methods: {
      ...
      //跳转详情页
      judgmentDetail(item){
        //uuid : item.uuid,simhash:item.simhash,infoid:item.infoId
        this.$router.push({path: 'pushwarningdetail', query: {uuid : item.uuid,simhash:item.simhash,infoid:item.infoId,openid:this.$store.state.userInfo.openid}});
      },
      //下拉加载
      onRefresh(){
        this.getList("screen");
      ...
      getList(s) {
        if (this.load) {
          ...
          if(this.listParameter.warningType == ""){
            if(this.$store.state.userInfo.judgedState !="1"){
              this.listParameter.warningType = "1,2,3,4,5";
            }else{
              this.listParameter.warningType = "1,2,3,4";
            }
          }
          getPushwarningList(this.listParameter).then(res => {
            Toast.clear();
      ...
      screenList(p){
        ...
        * */
        if(p.type=='time'){
          ...
          //预警级别
          this.listParameter.warningType = this.parameterType.warningType.val;
          //图片识别
      ...
      resetInfo(m){
        ...
        * */
        if(m.type == 'sourceType'){
          ...
          this.parameterType.warningType.val = '';
      ...
      closeItem(m){
        ...
        * */
        if(m.type == 'item_sourceType'){
          ...
          //筛选
          let wtlen = this.listParameter.warningType.length,
            judgedStates = this.$store.state.userInfo.judgedState;
          if((judgedStates!="1" && wtlen == 9) || (judgedStates =="1" && wtlen == 7)){
            this.parameterType.warningType.val = "";
          }else{
            this.parameterType.warningType.val = this.listParameter.warningType;
          }
...
<style scoped>
  @import url("../../judgment/list/list.css");
</style>
